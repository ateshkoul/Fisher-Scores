function FTable = featFisher(Data,initial_col,obj)
...
% the collective discriminative power at a time point. The function
% calculates only the collective (1:12) discriminative power.
...
for i = 1:10
    ...
    % point 
    featureLar{i} = Data(Data(:,obj)==2,initial_col+i);     
    featureSma{i} = Data(Data(:,obj)==1,initial_col+i);
...
%     featureSma{i} = Data(Data(:,2)==1,initial_col+i);
    ...
    meanFeatLar= nanmean(featureLar{1,i},1);
    meanFeatSma= nanmean(featureSma{1,i},1);
    ...
    for j=1:size(initial_col,2)
        F(j,i) = (norm(meanFeatLar(j)-meanFeatSma(j))).^2./(trace(cov(SLar(:,j)))+trace(cov(SSma(:,j))));
    end
    FAll(i) = (norm(meanFeatLar(1:12)-meanFeatSma(1:12))).^2./(trace(cov(SLar(:,1:12)))+trace(cov(SSma(:,1:12))));
...
% % A more General approach to calculate F-scores for each set of features.
% % This would help if we would like to know what combination of features has
% % the most discriminating power (rather than the question that at time 
% % point 1, what is the overall discriminating power of all the features).
% 
% 
...
%             check(i,j,k) = [peppa poppa];
%             check2(i,j,k)= [size(cov((SFeatSetSLar))) size(cov((SFeatSetSLar)))];
%             
%         end
%         
%         for l=1:12
